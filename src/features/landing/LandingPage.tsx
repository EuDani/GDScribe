    ...
    <div className="min-h-screen bg-ink text-paper">
      ...
      <section className="mx-auto max-w-4xl px-6 pt-20 pb-14 text-center sm:px-10">
        ...
        <div className="mt-14 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            ...
            <div key={label} className="border-2 border-ink bg-ink-soft px-3 py-4 shadow-brutal-sm">
              <div className="text-display text-2xl text-accent-yellow">{value}</div>
              <div className="text-label mt-1 text-[10px] text-paper/60">{label}</div>
            </div>
          ))}
        </div>
      </section>

      <section className="border-y-2 border-ink bg-ink-soft py-4">
        <div className="mx-auto flex max-w-4xl flex-wrap items-center justify-center gap-x-8 gap-y-2 px-6">
          <span className="text-label text-xs text-paper/50">Construído com</span>
          {STACK.map((s) => (
            <span key={s} className="text-label text-xs text-paper/80">
              {s}
            </span>
          ))}
        </div>
      ...
      <section id="features" className="mx-auto max-w-6xl px-6 py-20 sm:px-10">
        <div className="mb-10 text-center">
          <span className="text-label border-2 border-ink bg-accent-blue px-2 py-1 text-[11px] text-ink">
            Módulos
          </span>
          ...
          {FEATURES.map(({ icon: Icon, accent, title, description }, i) => (
            ...
            >
              <Card className="h-full">
                <div className={`mb-3 inline-flex border-2 border-ink p-2 text-ink ${accent}`}>
                  <Icon size={20} />
                </div>
                <h3 className="text-display mb-1.5 text-base">{title}</h3>
                <p className="text-sm text-paper/65">{description}</p>
              </Card>
            </motion.div>
          ))}
        </div>
      ...
      <section className="mx-auto max-w-4xl px-6 py-16 sm:px-10">
        <div className="mb-10 text-center">
          <span className="text-label border-2 border-ink bg-accent-green px-2 py-1 text-[11px] text-ink">
            Como funciona
          </span>
        ...
        <div className="space-y-4">
          {STEPS.map((step) => (
            <div key={step.n} className="flex items-start gap-4 border-2 border-ink bg-ink-soft p-5 shadow-brutal-sm">
              <span className="text-display shrink-0 text-3xl text-paper/25">{step.n}</span>
              <div>
                <h3 className="text-display text-base">{step.title}</h3>
                <p className="mt-1 text-sm text-paper/65">{step.description}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="border-t-2 border-ink px-6 py-20 text-center sm:px-10">
      ...
      <footer className="border-t-2 border-ink px-6 py-8 text-center sm:px-10">
        <p className="text-label text-xs text-paper/40">GDScribe — MIT License</p>
      </footer>
    </div>
  )
}
